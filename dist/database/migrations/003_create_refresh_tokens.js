...
async function up(db) {
    await db.schema
        ...
        .addForeignKeyConstraint('refresh_tokens_user_id_fk', ['user_id'], 'users', ['id'], (cb) => cb.onDelete('cascade'))
        .execute();
    await db.schema
    ...
    await db.schema
        ...
        .addColumn('user_id', 'uuid', (col) => col.notNull())
        .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `now()`))
        .addForeignKeyConstraint('project_members_project_id_fk', ['project_id'], 'projects', ['id'], (cb) => cb.onDelete('cascade'))
        .addForeignKeyConstraint('project_members_user_id_fk', ['user_id'], 'users', ['id'], (cb) => cb.onDelete('cascade'))
        .execute();
    await db.schema
...
//# sourceMappingURL=003_create_refresh_tokens.js.map